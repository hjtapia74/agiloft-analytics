...
class CustomerPage(FilteredPage):
    ...
    def render_sidebar_filters(self) -> Dict[str, Any]:
        ...
        try:
            ...
            with st.expander("Customer Selection", expanded=False):
                st.subheader("Customer ID Range")
                ...
                with col1:
                    customer_start = st.text_input(
                        ...
                        help="Enter the starting customer ID for your analysis range"
                    )
                ...
                with col2:
                    customer_end = st.text_input(
                        ...
                        help="Enter the ending customer ID for your analysis range"
                    )
    ...
    def validate_filters(self, filters: Dict[str, Any]) -> bool:
        ...
        try:
            start_num = int(customer_range[0].split('#')[1])
            end_num = int(customer_range[1].split('#')[1])
            if start_num >= end_num:
                self.render_warning("Start customer ID must be less than end customer ID")
                return False
        except:
            self.render_warning("Invalid customer ID format. Use: Customer#000000001")
            return False
    ...
    def process_data(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                FROM contract co
                JOIN customer c ON co.co_custkey = c.c_custkey
                WHERE c.c_name BETWEEN '{filters['customer_range'][0]}' AND '{filters['customer_range'][1]}'
